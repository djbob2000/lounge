...
async function getAlbumDetails(albumId: string): Promise<Album | null> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(
        `Error fetching album ${albumId}: ${res.status} ${res.statusText}`
      );
...
async function getAlbumPhotos(albumId: string): Promise<Photo[]> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error(
        `Error fetching photos for album ${albumId}: ${res.status} ${res.statusText}`
      );
    ...
    return (
      <div className="container mx-auto px-4 py-8 text-center">
        ...
        <p className="text-gray-600 mt-2">
          Album with ID <span className="font-mono">{albumId}</span> was not
          found. It might have been deleted or an incorrect ID was provided.
        </p>
