...
import AlbumActions from '../../../../components/admin/AlbumActions'; // Import AlbumActions

async function getAlbumDetails(albumId: string): Promise<Album | null> {
  const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api'; // Ensure /api prefix if needed
  try {
    const res = await fetch(`${apiUrl}/albums/${albumId}`, { cache: 'no-store' });
    if (!res.ok) {
      if (res.status === 404) {
...
async function getAlbumPhotos(albumId: string): Promise<Photo[]> {
  const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api'; // Ensure /api prefix if needed
  try {
    const res = await fetch(`${apiUrl}/photos/album/${albumId}`, { cache: 'no-store' });
    if (!res.ok) {
    ...
    return (
      <div className="container mx-auto px-4 py-8 text-center">
        ...
        <p className="text-gray-600 mt-2">
          Album with ID <span className="font-mono">{albumId}</span> was not found. It might have been deleted or an incorrect ID was provided.
        </p>
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <header className="mb-8">
        ...
        {album.description && (
          <p className="text-lg text-gray-600">{album.description}</p>
        )}
        <p className="text-sm text-gray-500 mt-1">Album ID: <span className="font-mono">{album.id}</span></p>
      </header>

      {/* AlbumActions will contain PhotoUploadForm, PhotoGrid, and photo deletion logic */}
      <AlbumActions albumId={album.id} initialPhotos={photos} />
