...
export default function PhotoUploadForm({
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    for (let i = 0; i < fileItems.length; i++) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          newProgress[i] =
            `❌ ${file.name} - Error: ${errorData.message || response.statusText}`;
          console.error(
        ...
        newProgress[i] =
          `❌ ${file.name} - Network error: ${error.message || 'Check connection'}`;
    ...
    setIsUploading(false);
    setOverallMessage(
      `Upload complete. Successful: ${successfulUploads}/${fileItems.length}.`
    );
